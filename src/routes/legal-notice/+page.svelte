...
<PageTransition>
    ...
    <p>
        {data.legalNotice.name}<br />
        {data.legalNotice.street}<br />
        {data.legalNotice.zipCity}<br />
        {data.legalNotice.country}<br />
    </p>

    <h2>Contact information</h2>
    <p>
        Phone: {data.legalNotice.phone}<br />
        E-Mail: {data.legalNotice.email}<br />
    </p>
    ...
    <p>
        ...
        content on these pages in accordance with § 7 paragraph 1 TMG under the
        general laws. According to §§ 8 to 10 TMG, however, we are not obligated
        ...
        the general laws remain unaffected. However, liability in this regard is
        only possible from the point in time at which a concrete infringement of
        the law becomes known. If we become aware of any such infringements, we
